...
{
    Buck_PID_Construct(&pid_u, 0.03, 0.002, 0, 10);
    Buck_PID_Construct(&pid_i, 0.03, 0.002, 0, 10);
    Buck_PID_SetClamp(&pid_i, 0.2, 0.8);
}
...
extern void buck_update(void)
{
    ...
    g_adc_in = u_out;
    //
    float u_err = v_ref - u_out;
    g_err = u_err;
    //
    float i_inp = Buck_PI_Transfer(&pid_u, u_err);

    float i_err = i_inp - getADCValue[2];

    float timer_d = Buck_PI_Transfer(&pid_i, i_err);
    g_out = timer_d;
    //
    PWM0init(timer_d);
}
